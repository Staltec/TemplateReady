...
// At Sat Nov 10 2012 16:11:39 GMT+0800 (KRAT)
...
jadeTestSelf: function(locals, attrs, escape, rethrow, merge) {
attrs = attrs || jade.attrs; escape = escape || jade.escape; rethrow = rethrow || jade.rethrow; merge = merge || jade.merge;
var buf = [];
var self = locals || {};
var interp;
buf.push('<div><h1>');
var __val__ = self.title
buf.push(escape(null == __val__ ? "" : __val__));
buf.push('</h1></div><ol>');
if ( (self.list.length))
{
...
buf.push('</ol><h2>Дискография: ' + escape((interp = self.hash.firstName) == null ? '' : interp) + ' ' + escape((interp = self.hash.lastName) == null ? '' : interp) + ' ' + escape((interp = self.hash.age ? '\('+self.hash.age+'\)' : '') == null ? '' : interp) + '</h2><ul>');
// iterate self.objList
;(function(){
  if ('number' == typeof self.objList.length) {

    for (var $index = 0, $$l = self.objList.length; $index < $$l; $index++) {
      var item = self.objList[$index];
...
buf.push('<li>' + escape((interp = item.year) == null ? '' : interp) + ' &mdash; ' + escape((interp = item.album) == null ? '' : interp) + '</li>');
    ...
    var $$l = 0;
    for (var $index in self.objList) {
      $$l++;      var item = self.objList[$index];
...
_n: {"underscore/test.htm":"underscoreTest","mustache/test space.mustache":"mustacheTest_space","mustache/test.mustache":"mustacheTest","jade/test.jade":"jadeTest","jade/testSelf.jade":"jadeTestSelf"},
